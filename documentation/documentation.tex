...
bool IsClockwise(const Triangle& triangle, const glm::vec3& polyCenter)
{
	...
	\begin{center}
		...
		\begin{tikzpicture}[tdplot_main_coords, scale=2]	
			...
			\coordinate (A) at (-0.7,-0.7,0.2);
			\coordinate (B) at (0.1, -0.35, 0.55);
			\coordinate (C) at (1.19, 0.13, 1.03);
			

			\draw[->, thick, blue] (A) -- (B) node[midway, above] {$\vec{n}$};
			\draw[dotted, thick, blue] (B) -- (C) node[right] {$r$};
			\filldraw[red] (A) circle (1pt) node[anchor=north east] {$a$};
