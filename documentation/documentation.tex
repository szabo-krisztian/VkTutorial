...
bool IsClockwise(const Triangle& triangle, const glm::vec3& polyCenter)
{
	...
	In this section, I will outline the methods and algorithms used in the development of the Cube Builder project. This includes a simplified 3D Digital Differential Analyzer (DDA) algorithm, ray-cube intersection techniques, and other relevant concepts.\\

	Before diving into the implementation details, let's first discuss the primary goal of this project. From the player's perspective, the concept is straightforward: you have a space divided into cubes where you can place and destroy blocks. The implementation works as follows: when the player wants to build or break a block, the first step is to identify the \textit{target} block, which is the cube the player is looking at. To achieve this, I initially used a DDA algorithm to determine all the cubes intersected by the ray defined by the player's position and facing direction. Next, I identified the closest block to the player that is already placed. If the player wants to break a block, the target block is destroyed. If the player wants to place a block, we determine which side of the target block the player is currently looking at and place a new block adjacent to it.
	
	\subsection{Intersection of cubes}
	First, let's explore a straightforward method for examining ray intersections with cubes. In this project, I used rays of finite length (more accurately referred to as segments). A ray can be defined by its starting point $a \in \R^3$ and direction vector $\vec{n} \in \R^3$.
	\[
	...
	These inequalities yield the following result: two intersection points, $s(t_1) = (-0.63, \, -1, \, 0.31)$ and $s(t_2) = (0, \, -0.27, \, 0.06)$, with parameters $t_1 = 0.377358$ and $t_2 = 0.652174$. Note that in my project, I only needed to determine whether any intersection points exist. For this purpose, we can use the following function.
...
bool DoesRayIntersectCube(const glm::vec3& rayStart, const glm::vec3& rayDirection, const glm::vec3& cubeMin, const glm::vec3& cubeMax)
{
	...
	The implementation of the algorithm I ultimately used can be found at the follwing  \href{https://github.com/francisengelmann/fast_voxel_traversal}{\textbf{\textcolor{blue}{GitHub repository}}}.
